...
# 一次性初始化 xxl_job 库
# 用法：在 cms-back 目录执行
#   chmod +x deploy/xxl-job/init-xxl-job-db.sh
#   ./deploy/xxl-job/init-xxl-job-db.sh

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/../../" && pwd)"
ENV_FILE="$ROOT_DIR/.env"
SQL_FILE="$(dirname "$0")/tables_xxl_job.sql"
...
if ! docker ps --format '{{.Names}}' | grep -qx "$MYSQL_CONTAINER"; then
  echo "容器 $MYSQL_CONTAINER 未运行，请先: docker compose up -d db" >&2
  exit 1
fi
...
echo "完成。可执行: docker compose up -d xxl-job-admin"